...
function setBGMText() {
    ...
    BGMText.innerHTML = `♫ <span id="BGMNameSelect">` + audioName + `</span>`;
    setTabName();
    ...
    try {
        if (playlistMode == 1) { // setup html for playlists (the previous/next buttons, the music count / playlist count)
            ...
            playlistText.innerHTML += `<button onclick="prevBGM()"><<< Previous</button> <button class="blankButton" onclick="pickRandomTrack()"> ${playlistNumPlaying + 1} / ${playlistLength} </button> <button onclick="nextBGM()">Next >>></button>`;
        }
    }
    catch (err) {
